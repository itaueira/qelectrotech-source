...
{
		/**
			The nine blocks todo/exemplos/macros/ delivers, with the counts
			BIBLIOTECA.md publishes beside each one. They are pinned here so
			that a block which loses a variable, or a library regenerated
			with a file missing, breaks the build instead of reaching a
			drawing board.
		*/
	struct DeliveredMacro
	{
		const char *file;
		int parameters;
		int value_sets;
	...
	const DeliveredMacro delivered_macros[] = {
		{ "acessorios-quadro.qetmak",              11, 2 },
		{ "conversor-dc-dc.qetmak",                10, 5 },
		{ "disjuntor-bipolar-distribuicao.qetmak",  7, 5 },
		{ "entrada-monofasica.qetmak",              5, 0 },
		{ "fonte-ac-dc-24v.qetmak",                 9, 5 },
		{ "monitor-fase-sinalizacao.qetmak",       13, 2 },
		{ "nobreak.qetmak",                        10, 5 },
		{ "placa-24-saidas.qetmak",                 5, 0 },
		{ "protecao-dps-saida.qetmak",              9, 5 } };

		/**
			The file as it is delivered, not a copy written into the test: a
			copy would go on passing after the delivered file broke, which is
			the one thing this test exists to catch.
		*/
	QDomDocument deliveredMacro(const char *file)
	{
		QFile handle(QStringLiteral(QET_TEST_DATA_DIR)
			     + QStringLiteral("/macros/")
			     + QString::fromUtf8(file));
		REQUIRE(handle.open(QIODevice::ReadOnly));
		const QString xml = QString::fromUtf8(handle.readAll());
		handle.close();

		QDomDocument document;
		REQUIRE(document.setContent(xml));
		REQUIRE(document.documentElement().tagName() == QStringLiteral("qet_macro"));
		return document;
	}

		/// The drawing, reached the way the insertion reaches it.
	QDomElement drawingOf(const QDomDocument &document)
	{
		return document.documentElement()
				.firstChildElement(QStringLiteral("diagram_content"))
				.firstChildElement(QStringLiteral("diagram"));
...
TEST_CASE("CU-09.1 — os nove blocos entregues são lidos pelo próprio programa", "[macro]")
{
		//A biblioteca da T09 é dado, não código, e dado quebra calado. Aqui
		//ela passa pelo mesmo leitor que o programa usa, e as contas que a
		//BIBLIOTECA.md publica são conferidas contra o arquivo entregue.
	for (const DeliveredMacro &delivered : delivered_macros)
	{
		INFO("bloco: " << delivered.file);
		const QDomDocument document = deliveredMacro(delivered.file);

		MacroParameterSet set;
		REQUIRE(set.fromXml(document.documentElement()));
		CHECK(set.count() == delivered.parameters);
		CHECK(int(set.valueSetNames().count()) == delivered.value_sets);
		CHECK_FALSE(drawingOf(document).isNull());
	}
}

TEST_CASE("CU-09.4 — toda variável da biblioteca tem rótulo, e o desenho a usa", "[macro]")
{
	for (const DeliveredMacro &delivered : delivered_macros)
	{
		INFO("bloco: " << delivered.file);
		const QDomDocument document = deliveredMacro(delivered.file);
		const QString xml = document.toString();

		MacroParameterSet set;
		REQUIRE(set.fromXml(document.documentElement()));

		for (const MacroParameter &parameter : set.parameters())
		{
			INFO("variável: " << parameter.name.toStdString());

			QString error;
			const bool valid = parameter.isValid(&error);
			INFO("recusa: " << error.toStdString());
			CHECK(valid);

				//Rótulo em branco vira caixa sem etiqueta no diálogo, e
				//rótulo igual ao nome deixa o projetista lendo
				//MARCACAO_DISJUNTOR em vez de "Marcação do disjuntor".
			CHECK_FALSE(parameter.label.isEmpty());
			CHECK(parameter.label != parameter.name);

				//Um padrão fora das escolhas abriria o diálogo já
				//recusando o próprio valor que ele mostra.
			CHECK(parameter.acceptsValue(parameter.default_value));

				//Variável declarada que o desenho não usa não muda nada
				//quando preenchida: é um campo a mais para ninguém.
			CHECK(xml.contains(parameter.marker()));
		}
	}
}

TEST_CASE("CU-09.2 — inserir com os padrões não deixa marcador para trás", "[macro]")
{
		//O que o CU-09.2 pede na tela é que alguém que não ajudou a montar a
		//biblioteca consiga usá-la. A parte disso que uma máquina prova é
		//esta: aceitar tudo como vem tem de dar um desenho inteiro, sem um
		//marcador desenhado na folha.
	for (const DeliveredMacro &delivered : delivered_macros)
	{
		INFO("bloco: " << delivered.file);
		const QDomDocument document = deliveredMacro(delivered.file);
		...
		QDomElement drawing = drawingOf(document);
		REQUIRE_FALSE(drawing.isNull());

		const MacroSubstitution::Result result =
			MacroSubstitution::apply(drawing, set.defaults());
		INFO("órfãos: " << result.orphans.join(QStringLiteral(", ")).toStdString());
		CHECK(result.ok);
		CHECK(result.orphans.isEmpty());
		CHECK(result.replacements > 0);

		QString rendered;
		QTextStream stream(&rendered);
		drawing.save(stream, 0);
		stream.flush();
		CHECK_FALSE(rendered.contains(QStringLiteral("${")));
	}
}

TEST_CASE("CU-09.3 — o conjunto por corrente troca as quatro de uma vez", "[macro]")
{
		//Os cinco disjuntores que a ACME comprou de verdade, na mesma
		//ordem em que a lista de compras os traz. Marca e modelo de peça
		//usada no projeto são livres; o que não se faz é amarrar uma
		//implementação à marca de terceiro.
	const struct
	{
		const char *set;
		const char *current;
		...
		{ "Disjuntor 2 A",  "2",  "MDWP-C2-2" },
		{ "Disjuntor 4 A",  "4",  "MDWP-C4-2" },
		{ "Disjuntor 6 A",  "6",  "MDWP-C6-2" },
		{ "Disjuntor 16 A", "16", "MDWP-C16-2" },
		{ "Disjuntor 20 A", "20", "MDWP-C20-2" } };

	const char *with_breaker[] = {
		"conversor-dc-dc.qetmak",
		"disjuntor-bipolar-distribuicao.qetmak",
		"fonte-ac-dc-24v.qetmak",
		"nobreak.qetmak",
		"protecao-dps-saida.qetmak" };

	SECTION("todo conjunto nomeia só variável declarada, e todos nomeiam as mesmas")
	{
		for (const DeliveredMacro &delivered : delivered_macros)
		{
			INFO("bloco: " << delivered.file);
			...
			QStringList first;
			for (const QString &name : set.valueSetNames())
			{
				INFO("conjunto: " << name.toStdString());
				const MacroValueSet value_set = set.valueSet(name);
				REQUIRE_FALSE(value_set.isNull());

					//Um valor para variável que ninguém declarou
					//sobrevive ao arquivo e não nomeia nada: seria um
					//conjunto que parece completo e não é.
				CHECK(set.undeclared(value_set.values).isEmpty());

					//É o que o CU-09.3 cobra: um conjunto que esqueceu
					//o código deixaria a corrente nova ao lado do
					//código velho, que é pior do que não ter conjunto.
				QStringList declared = value_set.values.keys();
				declared.sort();
				if (first.isEmpty())
					first = declared;
	...
	SECTION("escolher a corrente traz o código, a descrição e o fabricante juntos")
	{
		for (const char *file : with_breaker)
		{
			INFO("bloco: " << file);
			const QDomDocument document = deliveredMacro(file);

			MacroParameterSet set;
			REQUIRE(set.fromXml(document.documentElement()));

			const MacroParameter current =
				set.parameter(QStringLiteral("CORRENTE"));
			REQUIRE_FALSE(current.isNull());
			CHECK(current.type == MacroParameterType::List);

			for (const auto &breaker : breakers)
			{
				INFO("conjunto: " << breaker.set);
				const QString name = QString::fromUtf8(breaker.set);
				REQUIRE(set.hasValueSet(name));

					//A corrente do conjunto tem de ser uma das
					//escolhas da lista, senão o diálogo abre
					//recusando o valor que o conjunto acabou de pôr.
				CHECK(current.choices.contains(
					      QString::fromUtf8(breaker.current)));

				const QHash<QString, QString> applied =
					set.applyValueSet(name, set.defaults());
				CHECK(applied.value(QStringLiteral("CORRENTE"))
				      == QString::fromUtf8(breaker.current));
				CHECK(applied.value(QStringLiteral("CODIGO_DISJUNTOR"))
				      == QString::fromUtf8(breaker.code));
				CHECK(applied.value(QStringLiteral("FABRICANTE_DISJUNTOR"))
				      == QStringLiteral("WEG"));
				CHECK(applied.value(QStringLiteral("DESCRICAO_DISJUNTOR"))
				      .contains(QString::fromUtf8(breaker.code)));
			}
		}
	}

	SECTION("trocar de conjunto troca as quatro, e não leva junto o que é do bloco")
	{
		const QDomDocument document =
			deliveredMacro("disjuntor-bipolar-distribuicao.qetmak");

		MacroParameterSet set;
		REQUIRE(set.fromXml(document.documentElement()));

		const QHash<QString, QString> two =
			set.applyValueSet(QStringLiteral("Disjuntor 2 A"), set.defaults());
		const QHash<QString, QString> twenty =
			set.applyValueSet(QStringLiteral("Disjuntor 20 A"), two);

		CHECK(twenty.value(QStringLiteral("CORRENTE")) == QStringLiteral("20"));
		CHECK(twenty.value(QStringLiteral("CODIGO_DISJUNTOR"))
		      == QStringLiteral("MDWP-C20-2"));
		CHECK_FALSE(twenty.value(QStringLiteral("DESCRICAO_DISJUNTOR"))
			    .contains(QStringLiteral("MDWP-C2-2")));

			//A marcação e a borneira são do bloco, não do disjuntor: a
			//troca de conjunto não pode levá-las junto.
		CHECK(twenty.value(QStringLiteral("MARCACAO_DISJUNTOR"))
		      == two.value(QStringLiteral("MARCACAO_DISJUNTOR")));
		CHECK(twenty.value(QStringLiteral("BORNEIRA"))
		      == two.value(QStringLiteral("BORNEIRA")));
	}
}
